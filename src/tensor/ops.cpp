...
Tensor Tensor::matmul_2D(Tensor &other) {
  ...
  if (this->requires_grad || other.requires_grad) {
    ...
    ans.backward_fn = [](Tensor &self) {
      ...
      Tensor dZ{tensor_shape{{A.shape[0], B.shape[1]}}, tensor_data{self.grad}};
      if (A.requires_grad) {
...
Tensor Tensor::operator*(Tensor &other) { // Element wise multiplication , for
                                          // matrix multiplication use matmul
  if (this->shape.size() != other.shape.size())
    throw runtime_error("operator * :Index Dimensions Mismatch");
...
Tensor Tensor::sum() {
  ...
  (*out.get_data_pointer())[0] = accumulate(data.begin(), data.end(), 0.0f);
...
Tensor Tensor::leaky_relu(float alpha) {
  Tensor ans(tensor_shape{shape}, tensor_data{}, grad_flag{requires_grad});
  ...
  for (size_t i = 0; i < ans_data_pointer.size(); i++) {
    if (data[i] >= 0) {
      ans_data_pointer[i] = data[i];
    } else {
      ans_data_pointer[i] = alpha * data[i];
    }
  }

  if (requires_grad) {
    ans.parents = {this};
    ans.backward_fn = [alpha](Tensor &self) {
      Tensor &A = *self.parents[0];
      for (size_t i = 0; i < A.grad.size(); i++) {
